...
<head>
    <meta charset="utf-8">
    <title>شهادة نهاية التكوين - {{ $dossier->student->lastname }} {{ $dossier->student->firstname }}</title>
...
<div class="container">
        ...
        <div class="statement">
            <p>أشهد أن:</p>
            <p>السيد(ة): {{ $dossier->student->lastname_ar }} {{ $dossier->student->firstname_ar }}</p>
            <p>الحامل (ة) للبطاقة الوطنية للتعريف رقم: {{ $dossier->student->cin }}</p>
            <p>الرقم الممنوح من طرف الإدارة web ref: 5798-{{ $dossier->student->cin }}-{{ $dossier->ref }}</p>
